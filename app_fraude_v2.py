"""
app_fraude.py - Aplicación Streamlit para Detección de Fraude Médico

Interfaz web interactiva para analizar reclamos médicos y detectar posibles fraudes
usando las herramientas de herramientas_fraude_avanzado_v2.py

Instalación:
pip install streamlit plotly pandas numpy scikit-learn sentence-transformers joblib

Uso:
streamlit run app_fraude.py
"""
...
try:
    from herramientas_fraude_avanzado_v2 import (
    ...
    st.info("Asegúrate de que 'herramientas_fraude_avanzado_v2.py' esté en el mismo directorio")
...
with tab1:
    ...
    with st.form("form_analisis"):
        ...
        with col1:
            texto_reclamo = st.text_area(
                ...
                placeholder="Ingrese el texto completo del reclamo médico...\n\n"
                           "Ejemplo: Paciente de 55 años con dolor crónico en hombro derecho. "
                           "Diagnóstico: Hombro congelado (M75.0). Procedimiento: Artroscopia "
                           "(CPT 29826). Monto reclamado: $2.500.000. Incapacidad: 90 días.",
                help="Incluya toda la información disponible: diagnósticos, procedimientos, síntomas, etc."
            )

        with col2:
            st.subheader("Datos Adicionales (Opcional)")
        ...
        submitted = st.form_submit_button("🔎 Analizar Reclamo",
                                          type="primary",
        ...
        with st.spinner("🔍 Analizando reclamo..."):
            try:
                ...
                if os.path.exists(MODELO_FRAUDE_PATH) and SKLEARN_AVAILABLE:
                    try:
                        ...
                        pred = predecir_modelo(
                            monto=monto if monto > 0 else 5000,
                            edad=edad,
                ...
                if prob_modelo is not None:
                    # Combinar heurística y modelo (60% heurística, 40% modelo)
                    prob_final = 0.6 * prob_heuristica + 0.4 * prob_modelo
                else:
...
with tab4:
    ...
    3. Opcionalmente, agrega datos adicionales (monto, edad, días de incapacidad)
