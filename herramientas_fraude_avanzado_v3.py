...
PALABRAS_SOSPECHOSAS = [
    ...
    "fraude", "reclamo sospechoso", "cobro indebido", "incapacidad permanente",
...
def detectar_fraude_heuristico(reclamo: str) -> Dict[str, Any]:
    ...
    prob = (0.30 * score_kw) + (0.30 * score_dx) + (0.2 * score_cpt) + (0.2 * score_sem)
...
def entrenar_modelo_sintetico(n_samples: int = 500, random_state: int = 42) -> Dict[str, Any]:
    ...
    Incluye casos de fraude con montos y días elevados al azar.
    Montos en pesos chilenos (CLP).
    Requiere scikit-learn. Guarda modelo en disco.
    """
    ...
    # Generación de datos con distribución realista para Chile (CLP)
    # 85% casos normales + 15% casos con valores elevados (potenciales fraudes)

    # Montos en pesos chilenos
    montos_normales = rng.exponential(scale=1_800_000, size=int(n_samples * 0.85))  # ~500K-3M CLP
    montos_elevados = rng.uniform(7_000_000, 22_000_000, size=int(n_samples * 0.15))  # 7M-22M CLP sospechosos
    montos = np.concatenate([montos_normales, montos_elevados])
    rng.shuffle(montos)

    # Edades realistas
    edades = rng.randint(18, 90, size=n_samples)

    # Días de incapacidad: mayoría normal + algunos muy largos (sospechosos)
    dias_normales = rng.randint(0, 90, size=int(n_samples * 0.85))  # Mayoría 0-90 días
    dias_elevados = rng.randint(180, 365, size=int(n_samples * 0.15))  # Incapacidades muy largas
    dias = np.concatenate([dias_normales, dias_elevados])
    rng.shuffle(dias)

    # Keywords y códigos especiales
    cnt_kw = rng.poisson(0.3, size=n_samples)
    has_cpt5 = rng.binomial(1, 0.08, size=n_samples)

    # Lógica de fraude mejorada para Chile:
    # 1. Montos muy elevados (>9M CLP) son altamente sospechosos
    # 2. Días muy largos (>200) son sospechosos
    # 3. Combinación de monto alto (>5.5M) + días largos (>120) es fraude probable
    # 4. Casos con códigos especiales + múltiples keywords también pueden ser fraude
    fraude_monto_extremo = montos > 9_000_000  # >9M CLP
    fraude_dias_extremos = dias > 200  # >200 días
    fraude_combinado = (montos > 5_500_000) & (dias > 100)  # >5.5M + días largos
    fraude_codigos = (has_cpt5 == 1) & (cnt_kw > 1)  # Códigos especiales + keywords

    # Combinar condiciones (al menos una debe cumplirse para ser fraude)
    y = fraude_monto_extremo | fraude_dias_extremos | fraude_combinado | fraude_codigos

    # Añadir ruido realista para simular mundo real:
    # - Algunos fraudes reales no se detectan (falsos negativos) ~5%
    # - Algunos casos normales parecen fraude (falsos positivos) ~3%
    mascara_fn = rng.random(n_samples) < 0.05
    y[mascara_fn & (y == 1)] = 0  # Algunos fraudes pasan desapercibidos

    mascara_fp = rng.random(n_samples) < 0.03
    y[mascara_fp & (y == 0)] = 1  # Algunos casos normales se marcan como fraude
    ...
    # Entrenamiento con class balancing
    ...
    # Calcular estadísticas de fraudes generados
    pct_fraude = (y.sum() / len(y)) * 100
    monto_promedio_fraude = montos[y == 1].mean() if y.sum() > 0 else 0
    monto_promedio_normal = montos[y == 0].mean() if (y == 0).sum() > 0 else 0
    ...
    return {
        "n_samples": n_samples,
        "n_fraudes": int(y.sum()),
        "n_normales": int((y == 0).sum()),
        "pct_fraude": round(pct_fraude, 2),
        "auc_test": float(round(auc, 4)),
        "monto_promedio_fraude_clp": int(monto_promedio_fraude),
        "monto_promedio_normal_clp": int(monto_promedio_normal),
        "modelo_guardado": MODELO_FRAUDE_PATH,
        "nota": "Montos en pesos chilenos (CLP)"
    }


def predecir_modelo(monto: float, edad: int, dias_incapacidad: int,
                    cnt_keywords: int = 0, has_cpt5: int = 0) -> Dict[str, Any]:
    """
    Predice usando el modelo entrenado sintético.
    Retorna probabilidad de fraude y categorización del riesgo.

    Args:
        monto: Monto del reclamo en pesos chilenos (CLP)
        edad: Edad del paciente
        dias_incapacidad: Días de incapacidad laboral
        cnt_keywords: Cantidad de palabras clave sospechosas detectadas
        has_cpt5: 1 si tiene códigos CPT de 5+ dígitos (procedimientos caros), 0 si no
    """
    if not SKLEARN_AVAILABLE:
    ...
    if prob >= 0.7:
        riesgo = "ALTO"
        accion_sugerida = "Requiere investigación inmediata"
    elif prob >= 0.4:
        riesgo = "MEDIO"
        accion_sugerida = "Requiere revisión adicional"
    else:
        riesgo = "BAJO"
        accion_sugerida = "Procesamiento normal"

    # Flags adicionales para alertas
    alertas = []
    if monto > 9_000_000:
        alertas.append("Monto extremadamente alto (>9M CLP)")
    if dias_incapacidad > 200:
        alertas.append("Incapacidad excesivamente larga (>200 días)")
    if monto > 5_500_000 and dias_incapacidad > 120:
        alertas.append("Combinación sospechosa: monto alto + días largos")

    return {
        "probabilidad_modelo": round(prob, 3),
        "nivel_riesgo": riesgo,
        ...
        "datos_entrada": {
            ...
            "keywords_detectadas": cnt_keywords,
            "tiene_cpt_caro": bool(has_cpt5)
        }
    }
    ...
    if args.predecir:
        ...
        if SKLEARN_AVAILABLE and joblib and os.path.exists(MODELO_FRAUDE_PATH):
            ...
            try:
                ...
                sample = predecir_modelo(monto=5_000_000, edad=45, dias_incapacidad=90, cnt_keywords=cnt_kw, has_cpt5=has_cpt5)
